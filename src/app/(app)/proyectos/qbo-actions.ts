...
// ── Asignar cotizaciones a un proyecto ───────────────────────────────────────
//
// La relación ya existía y es 1 proyecto → N cotizaciones: vive en
// `sales_quotes.qbo_project_no`. Lo que faltaba era armarla DESDE el proyecto;
// hasta ahora solo se podía cotización por cotización desde Cotizaciones, y hay
// 267 sin asignar. No se crea tabla nueva: sigue habiendo una sola fuente.

export type CotizacionAsignable = {
  id: string;
  quoteNumber: string;
  ...
  source: string | null;
  projectNo: string | null; // proyecto al que está asignada hoy (null = libre)
};

export async function listCotizacionesAsignables(): Promise<CotizacionAsignable[]> {
  const supabase = await createClient();
  const orgId = await getActiveOrgId();
  if (!orgId) return [];
  const { data, error } = await supabase
    .from("sales_quotes")
    .select("id, quote_number, client_name, client_std_name, description, amount_usd, status, sent_date, source, qbo_project_no")
    .eq("org_id", orgId)
    .order("sent_date", { ascending: false, nullsFirst: false })
    .limit(2000);
  if (error || !data) return [];
  type Row = {
    id: string; quote_number: string | null; client_name: string | null; client_std_name: string | null;
    description: string | null; amount_usd: number | null; status: string | null; sent_date: string | null;
    source: string | null; qbo_project_no: string | null;
  };
  return (data as Row[]).map((r) => ({
    id: r.id,
    quoteNumber: r.quote_number ?? "(sin número)",
    clientName: r.client_std_name ?? r.client_name,
    description: r.description,
    amount: r.amount_usd,
    status: r.status,
    sentDate: r.sent_date,
    source: r.source,
    projectNo: r.qbo_project_no?.trim() || null,
  }));
}

// Asignar/soltar en una sola llamada. `codigo` null suelta la cotización.
// Una cotización pertenece a UN proyecto: asignarla a otro la mueve, y la UI lo
// avisa antes en vez de hacerlo callada.
export async function asignarCotizaciones(
  ids: string[],
  codigo: string | null,
): Promise<{ ok: true } | { ok: false; error: string }> {
  const supabase = await createClient();
  ...
  const { error } = await supabase
    .from("sales_quotes")
    .update({ qbo_project_no: codigo })
    .in("id", ids)
    .eq("org_id", orgId);
  if (error) {
    return {
      ok: false,
      error: /qbo_project_no/.test(error.message)
        ? "Falta la migración 0037 en Supabase — corre el SQL y reintenta."
        : error.message,
    };
  }
  return { ok: true };
