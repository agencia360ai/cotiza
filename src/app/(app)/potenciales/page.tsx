...
import { listQuotes, listProjectOptions } from "@/lib/pipeline/queries";
...
export default async function PotencialesPage({
  ...
  const [quotes, projectOptions] = await Promise.all([listQuotes(orgId), listProjectOptions(orgId)]);
  ...
  return <PotencialesScreen quotes={quotes} clients={clients} projectOptions={projectOptions} initialQuery={q ?? ""} />;
}
